...
def unite_selectbox(label, key, default=""):
    ...
    opts=UNITES_MESURE
    # Si la valeur actuelle n'est pas dans la liste, l'ajouter temporairement
...
def init_db():
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS dossiers(id INTEGER PRIMARY KEY AUTOINCREMENT,nom TEXT NOT NULL UNIQUE,description TEXT DEFAULT '',client TEXT DEFAULT '',date_creation TEXT DEFAULT '',statut TEXT DEFAULT 'En cours',observation TEXT DEFAULT '')""")
    cur.execute("""CREATE TABLE IF NOT EXISTS rues(id INTEGER PRIMARY KEY AUTOINCREMENT,dossier_id INTEGER,nom TEXT NOT NULL,zone TEXT DEFAULT '',longueur_m REAL DEFAULT 0,largeur_m REAL DEFAULT 0,observation TEXT DEFAULT '',numero_marche TEXT DEFAULT '',objet_marche TEXT DEFAULT '',maitre_ouvrage TEXT DEFAULT '',maitre_ouvrage_delegue TEXT DEFAULT '',entreprise TEXT DEFAULT '',bureau_controle TEXT DEFAULT '',labo TEXT DEFAULT '',coordinateur_securite TEXT DEFAULT '',date_notification TEXT DEFAULT '',date_demarrage TEXT DEFAULT '',delai_jours REAL DEFAULT 0,statut_chantier TEXT DEFAULT 'En cours',FOREIGN KEY(dossier_id) REFERENCES dossiers(id) ON DELETE SET NULL)""")
    cur.execute("""CREATE TABLE IF NOT EXISTS livrables(id INTEGER PRIMARY KEY AUTOINCREMENT,chantier_id INTEGER NOT NULL,nom TEXT NOT NULL,type_livrable TEXT DEFAULT 'Rue',description TEXT DEFAULT '',longueur_m REAL DEFAULT 0,largeur_m REAL DEFAULT 0,observation TEXT DEFAULT '',FOREIGN KEY(chantier_id) REFERENCES rues(id) ON DELETE CASCADE)""")
    cur.execute("""CREATE TABLE IF NOT EXISTS devis_rue(id INTEGER PRIMARY KEY AUTOINCREMENT,rue_id INTEGER NOT NULL,livrable_id INTEGER,code_poste TEXT DEFAULT '',designation TEXT NOT NULL,unite TEXT NOT NULL,quantite_marche REAL DEFAULT 0,prix_unitaire REAL DEFAULT 0,observation TEXT DEFAULT '',FOREIGN KEY(rue_id) REFERENCES rues(id) ON DELETE CASCADE,FOREIGN KEY(livrable_id) REFERENCES livrables(id) ON DELETE SET NULL)""")
    cur.execute("""CREATE TABLE IF NOT EXISTS realisations_journalieres(id INTEGER PRIMARY KEY AUTOINCREMENT,date_suivi TEXT NOT NULL,rue_id INTEGER NOT NULL,devis_id INTEGER NOT NULL,quantite_jour REAL DEFAULT 0,observation TEXT DEFAULT '',FOREIGN KEY(rue_id) REFERENCES rues(id),FOREIGN KEY(devis_id) REFERENCES devis_rue(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS sous_traitants(id INTEGER PRIMARY KEY AUTOINCREMENT,nom TEXT NOT NULL UNIQUE,specialite TEXT DEFAULT '',responsable TEXT DEFAULT '',telephone TEXT DEFAULT '',email TEXT DEFAULT '',montant_contrat REAL DEFAULT 0,date_debut TEXT DEFAULT '',date_fin TEXT DEFAULT '',statut TEXT DEFAULT 'Actif',rue_id INTEGER,observation TEXT DEFAULT '',FOREIGN KEY(rue_id) REFERENCES rues(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS devis_st(id INTEGER PRIMARY KEY AUTOINCREMENT,st_id INTEGER NOT NULL,rue_id INTEGER,code_poste TEXT DEFAULT '',designation TEXT NOT NULL,unite TEXT NOT NULL,quantite REAL DEFAULT 0,prix_unitaire REAL DEFAULT 0,observation TEXT DEFAULT '',FOREIGN KEY(st_id) REFERENCES sous_traitants(id) ON DELETE CASCADE,FOREIGN KEY(rue_id) REFERENCES rues(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS decomptes_st(id INTEGER PRIMARY KEY AUTOINCREMENT,st_id INTEGER NOT NULL,rue_id INTEGER,numero_decompte INTEGER DEFAULT 1,date_decompte TEXT NOT NULL,devis_st_id INTEGER,quantite_executee REAL DEFAULT 0,montant REAL DEFAULT 0,observation TEXT DEFAULT '',valide INTEGER DEFAULT 0,FOREIGN KEY(st_id) REFERENCES sous_traitants(id),FOREIGN KEY(devis_st_id) REFERENCES devis_st(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS paiements_st(id INTEGER PRIMARY KEY AUTOINCREMENT,st_id INTEGER NOT NULL,rue_id INTEGER,date_paiement TEXT NOT NULL,montant REAL NOT NULL,reference TEXT DEFAULT '',mode_paiement TEXT DEFAULT 'Virement',observation TEXT DEFAULT '',FOREIGN KEY(st_id) REFERENCES sous_traitants(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS personnel(id INTEGER PRIMARY KEY AUTOINCREMENT,nom TEXT NOT NULL,prenom TEXT DEFAULT '',categorie TEXT NOT NULL,poste TEXT DEFAULT '',salaire_journalier REAL DEFAULT 0,telephone TEXT DEFAULT '',date_entree TEXT DEFAULT '',actif INTEGER DEFAULT 1,rue_id INTEGER,observation TEXT DEFAULT '',FOREIGN KEY(rue_id) REFERENCES rues(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS pointage(id INTEGER PRIMARY KEY AUTOINCREMENT,date_pointage TEXT NOT NULL,personnel_id INTEGER NOT NULL,rue_id INTEGER,statut TEXT NOT NULL DEFAULT 'Présent',heures_travaillees REAL DEFAULT 8,tache TEXT DEFAULT '',observation TEXT DEFAULT '',FOREIGN KEY(personnel_id) REFERENCES personnel(id),FOREIGN KEY(rue_id) REFERENCES rues(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS materiaux(id INTEGER PRIMARY KEY AUTOINCREMENT,nom TEXT NOT NULL UNIQUE,unite TEXT NOT NULL,categorie TEXT DEFAULT 'Matériau',stock_initial REAL DEFAULT 0,seuil_alerte REAL DEFAULT 0,prix_unitaire REAL DEFAULT 0)""")
    cur.execute("""CREATE TABLE IF NOT EXISTS approvisionnements(id INTEGER PRIMARY KEY AUTOINCREMENT,date_besoin TEXT NOT NULL,rue_id INTEGER,materiau_id INTEGER,designation TEXT NOT NULL,unite TEXT NOT NULL,quantite_demandee REAL DEFAULT 0,prix_unitaire_estime REAL DEFAULT 0,demandeur TEXT DEFAULT '',motif TEXT DEFAULT '',statut TEXT DEFAULT 'Besoin exprimé',date_validation_cc TEXT DEFAULT '',validateur_cc TEXT DEFAULT '',numero_bc TEXT DEFAULT '',date_bc TEXT DEFAULT '',fournisseur TEXT DEFAULT '',date_reception TEXT DEFAULT '',quantite_recue REAL DEFAULT 0,bon_livraison TEXT DEFAULT '',date_mise_stock TEXT DEFAULT '',quantite_mise_stock REAL DEFAULT 0,prix_unitaire_reel REAL DEFAULT 0,observation TEXT DEFAULT '',FOREIGN KEY(rue_id) REFERENCES rues(id),FOREIGN KEY(materiau_id) REFERENCES materiaux(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS mouvements_materiaux(id INTEGER PRIMARY KEY AUTOINCREMENT,date_mvt TEXT NOT NULL,rue_id INTEGER,materiau_id INTEGER NOT NULL,type_mvt TEXT NOT NULL,quantite REAL NOT NULL,prix_unitaire REAL DEFAULT 0,fournisseur TEXT DEFAULT '',bon_livraison TEXT DEFAULT '',appro_id INTEGER,observation TEXT DEFAULT '',FOREIGN KEY(rue_id) REFERENCES rues(id),FOREIGN KEY(materiau_id) REFERENCES materiaux(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS materiels(id INTEGER PRIMARY KEY AUTOINCREMENT,nom TEXT NOT NULL UNIQUE,type_materiel TEXT DEFAULT '',immatriculation TEXT DEFAULT '',marque TEXT DEFAULT '',annee INTEGER DEFAULT 0,cout_horaire REAL DEFAULT 0,cout_journalier REAL DEFAULT 0,statut TEXT DEFAULT 'Disponible',rue_id_affectation INTEGER,date_derniere_maintenance TEXT DEFAULT '',prochain_entretien_heures REAL DEFAULT 0,heures_totales REAL DEFAULT 0,observation TEXT DEFAULT '',FOREIGN KEY(rue_id_affectation) REFERENCES rues(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS suivi_materiels(id INTEGER PRIMARY KEY AUTOINCREMENT,date_suivi TEXT NOT NULL,rue_id INTEGER,materiel_id INTEGER NOT NULL,heures_marche REAL DEFAULT 0,heures_arret REAL DEFAULT 0,carburant_materiau_id INTEGER,carburant_l REAL DEFAULT 0,cout_carburant REAL DEFAULT 0,chauffeur TEXT DEFAULT '',kilometre_debut REAL DEFAULT 0,kilometre_fin REAL DEFAULT 0,panne TEXT DEFAULT '',observation TEXT DEFAULT '',FOREIGN KEY(rue_id) REFERENCES rues(id),FOREIGN KEY(materiel_id) REFERENCES materiels(id),FOREIGN KEY(carburant_materiau_id) REFERENCES materiaux(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS maintenance_materiels(id INTEGER PRIMARY KEY AUTOINCREMENT,materiel_id INTEGER NOT NULL,date_maintenance TEXT NOT NULL,type_maintenance TEXT DEFAULT 'Préventive',description TEXT NOT NULL,cout REAL DEFAULT 0,prestataire TEXT DEFAULT '',pieces_changees TEXT DEFAULT '',heures_compteur REAL DEFAULT 0,prochain_entretien_h REAL DEFAULT 0,observation TEXT DEFAULT '',FOREIGN KEY(materiel_id) REFERENCES materiels(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS journal_chantier(id INTEGER PRIMARY KEY AUTOINCREMENT,date_journal TEXT NOT NULL,rue_id INTEGER,meteo TEXT DEFAULT '',nb_ouvriers_presents INTEGER DEFAULT 0,travaux_executes TEXT DEFAULT '',problemes TEXT DEFAULT '',decisions TEXT DEFAULT '',visiteurs TEXT DEFAULT '',redacteur TEXT DEFAULT '',observation TEXT DEFAULT '',FOREIGN KEY(rue_id) REFERENCES rues(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS caisse_chantier(id INTEGER PRIMARY KEY AUTOINCREMENT,date_op TEXT NOT NULL,categorie TEXT DEFAULT 'CHANTIER',rue_id INTEGER,type_op TEXT NOT NULL,rubrique TEXT NOT NULL,montant REAL NOT NULL,beneficiaire TEXT DEFAULT '',reference_piece TEXT DEFAULT '',mode_paiement TEXT DEFAULT 'Espèces',valide INTEGER DEFAULT 0,observation TEXT DEFAULT '',FOREIGN KEY(rue_id) REFERENCES rues(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS courriers(id INTEGER PRIMARY KEY AUTOINCREMENT,date_courrier TEXT NOT NULL,rue_id INTEGER,type_courrier TEXT NOT NULL,sens TEXT NOT NULL,reference TEXT DEFAULT '',objet TEXT NOT NULL,expediteur_destinataire TEXT DEFAULT '',statut TEXT DEFAULT 'En cours',observation TEXT DEFAULT '',FOREIGN KEY(rue_id) REFERENCES rues(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS incidents(id INTEGER PRIMARY KEY AUTOINCREMENT,date_incident TEXT NOT NULL,rue_id INTEGER,type_incident TEXT NOT NULL,gravite TEXT DEFAULT 'Mineur',description TEXT NOT NULL,personne_concernee TEXT DEFAULT '',mesures_prises TEXT DEFAULT '',cloture INTEGER DEFAULT 0,observation TEXT DEFAULT '',FOREIGN KEY(rue_id) REFERENCES rues(id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS audit_trail(id INTEGER PRIMARY KEY AUTOINCREMENT,date_action TEXT NOT NULL,table_concernee TEXT NOT NULL,action TEXT NOT NULL,enregistrement_id INTEGER,details TEXT DEFAULT '')""")
    # migrations — ajout de toutes les colonnes manquantes pour compatibilité ascendante
    migs={
        "rues":[
            ("dossier_id","INTEGER","NULL"),("statut_chantier","TEXT","'En cours'"),
            ("maitre_ouvrage_delegue","TEXT","''"),("bureau_controle","TEXT","''"),
            ("labo","TEXT","''"),("coordinateur_securite","TEXT","''"),
            ("delai_mois","REAL","0"),("date_notification","TEXT","''"),("date_demarrage","TEXT","''"),
        ],
        "devis_rue":[("livrable_id","INTEGER","NULL")],
        "materiaux":[("categorie","TEXT","'Matériau'"),("seuil_alerte","REAL","0"),("prix_unitaire","REAL","0")],
        # materiels : la table ancienne a 'statut' — on ajoute 'etat' + colonnes manquantes
        "materiels":[
            ("etat","TEXT","'Opérationnel'"),
            ("immatriculation","TEXT","''"),
            ("type_materiel","TEXT","''"),
            ("date_acquisition","TEXT","''"),
            ("heure_compteur","REAL","0"),
            ("observations","TEXT","''"),
            ("marque","TEXT","''"),("annee","INTEGER","0"),
            ("rue_id_affectation","INTEGER","NULL"),
            ("date_derniere_maintenance","TEXT","''"),
            ("prochain_entretien_heures","REAL","0"),("heures_totales","REAL","0"),
        ],
        # suivi_materiels : ajouter heures_travail, carburant_consomme, observations
        "suivi_materiels":[
            ("carburant_materiau_id","INTEGER","NULL"),
            ("heures_travail","REAL","0"),
            ("carburant_consomme","REAL","0"),
            ("observations","TEXT","''"),
            ("cout_carburant","REAL","0"),("kilometre_debut","REAL","0"),
            ("kilometre_fin","REAL","0"),("heures_arret","REAL","0"),
            ("carburant_l","REAL","0"),("panne","TEXT","''"),("chauffeur","TEXT","''"),
        ],
        # journal_chantier : colonnes utilisées par le code v3
        "journal_chantier":[
            ("rue_id","INTEGER","NULL"),
            ("meteo","TEXT","''"),("temperature","REAL","20"),
            ("nb_ouvriers","INTEGER","0"),("nb_encadrants","INTEGER","0"),
            ("travaux_realises","TEXT","''"),("observations","TEXT","''"),
        ],
        # caisse_chantier : noms de colonnes v3
        "caisse_chantier":[
            ("montant","REAL","0"),("beneficiaire","TEXT","''"),
            ("reference_piece","TEXT","''"),("observations","TEXT","''"),
            ("mode_paiement","TEXT","'Espèces'"),("valide","INTEGER","0"),
            ("categorie","TEXT","'CHANTIER'"),("rue_id","INTEGER","NULL"),
        ],
        # courriers : colonnes v3
        "courriers":[
            ("rue_id","INTEGER","NULL"),
            ("priorite","TEXT","'Normale'"),("expediteur","TEXT","''"),
            ("destinataire","TEXT","''"),("resume","TEXT","''"),
            ("actions_requises","TEXT","''"),("statut","TEXT","'Nouveau'"),
            ("objet","TEXT","''"),("type_courrier","TEXT","'Entrant'"),
            ("date_courrier","TEXT","''"),("reference","TEXT","''"),
        ],
        # incidents : colonnes v3
        "incidents":[
            ("rue_id","INTEGER","NULL"),
            ("nb_victimes","INTEGER","0"),("cout_estime","REAL","0"),
            ("actions_correctives","TEXT","''"),("statut","TEXT","'Ouvert'"),
            ("type_incident","TEXT","''"),("gravite","TEXT","'Mineur'"),
            ("description","TEXT","''"),("date_incident","TEXT","''"),
        ],
        # audit_log : créer si manquant (géré via CREATE TABLE ci-dessous)
        "sous_traitants":[("email","TEXT","''"),("rue_id","INTEGER","NULL")],
        "personnel":[("rue_id","INTEGER","NULL")],
        "mouvements_materiaux":[("bon_livraison","TEXT","''"),("appro_id","INTEGER","NULL")],
        "devis_st":[("rue_id","INTEGER","NULL"),("quantite","REAL","0")],
        "decomptes_st":[("rue_id","INTEGER","NULL"),("devis_st_id","INTEGER","NULL"),
                        ("quantite_executee","REAL","0"),("montant","REAL","0"),("valide","INTEGER","0")],
        "paiements_st":[("rue_id","INTEGER","NULL"),("mode_paiement","TEXT","'Virement'")],
        "pointage":[("heures_normales","REAL","8"),("heures_sup","REAL","0")],
    }
    # Créer audit_log si absent
    cur.execute("""CREATE TABLE IF NOT EXISTS audit_log(id INTEGER PRIMARY KEY AUTOINCREMENT,
        timestamp TEXT NOT NULL, table_name TEXT NOT NULL, action TEXT NOT NULL,
    ...
    for table,cols in migs.items():
        try:
            ...
            for col,typ,dflt in cols:
                if col not in ex: cur.execute(f"ALTER TABLE {table} ADD COLUMN {col} {typ} DEFAULT {dflt}")
        except Exception: pass
    # Migration spéciale : copier quantite_marche → quantite dans devis_st si nécessaire
    try:
        cur.execute("PRAGMA table_info(devis_st)")
        dst_cols={r[1] for r in cur.fetchall()}
        if "quantite_marche" in dst_cols and "quantite" in dst_cols:
            cur.execute("UPDATE devis_st SET quantite=quantite_marche WHERE quantite=0 AND quantite_marche>0")
    except Exception: pass
...
def qdf(sql,p=None):
    c=get_conn()
    try: df=pd.read_sql_query(sql,c,params=p or [])
    except Exception: df=pd.DataFrame()
    finally: c.close()
    return df

def exsql(sql,p=None):
    c=get_conn(); cur=c.cursor(); cur.execute(sql,p or []); lid=cur.lastrowid; c.commit(); c.close(); return lid

def exmany(sql,rows):
    c=get_conn(); cur=c.cursor(); cur.executemany(sql,rows); c.commit(); c.close()
